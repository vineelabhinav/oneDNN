...
void jit_brgemm_matmul_copy_b_transposed_t<sve_256>::copy_row_x_col(
    ...
    auto vpalgr = [&](ZReg dst, ZReg src1, ZReg src2, int a)
    {

        //lower half of dst in dst
        
        mov(z_tmp_0.d,src2.d);
        splice(z_tmp_0.s,p_0F,src1.s);


        mov(dst.d, z_tmp_0.d);
        ext(dst.b, z_tmp_0.b, a);

        //upper half of dst in z_tmp_1

        mov(z_tmp_0.d, src2.d);
        ext(z_tmp_0.b, src2.b, 16);

        mov(z_tmp_0.s,p_F0,src1.s);

        mov(z_tmp_1.d, z_tmp_0.d);
        ext(z_tmp_1.b, z_tmp_0.b, a);

        //merge dst lower and upper
        splice(dst.s,p_0F, z_tmp_1.s); 
